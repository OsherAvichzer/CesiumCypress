describe("tests for", () => {
  beforeEach("Pin by coordinate", () => {
    // Visit the page
    cy.visit(
      "https://sandcastle.cesium.com/standalone.html#c="
    );
    cy.zoomToTLV();
    //screenshot to map (before adding a point)
    cy.screenshot(`baseImg`);
  });

  it("comparing after pinning a point", () => {
    cy.pinInTLV();
    cy.screenshot(`pointImg`);

    cy.readFile(
      `C:/CesiumCypress/Documentation/Screenshots/baseImg.png`,
      "base64"
    ).then((mapBaseImg) => {
      cy.readFile(
        `C:/CesiumCypress/Documentation/Screenshots/pointImg.png`,
        "base64"
      ).then((editedMapImg) => {
        cy.getPixelDiff(mapBaseImg, editedMapImg).then((diffPrecent) => {
          expect(diffPrecent).to.be.above(0.2);
        });
      });
    });
  });

  it("comparing without pinning a point", () => {
    cy.screenshot(`withoutPointImg`);

    cy.readFile(
      `C:/CesiumCypress/Documentation/Screenshots/baseImg.png`,
      "base64"
    ).then((mapBaseImg) => {
      cy.readFile(
        `C:/CesiumCypress/Documentation/Screenshots/withoutPointImg.png`,
        "base64"
      ).then((editedMapImg) => {
        cy.getPixelDiff(mapBaseImg, editedMapImg).then((diffPrecent) => {
          expect(diffPrecent).to.be.above(0.2);
        });
      });
    });
  });
});
