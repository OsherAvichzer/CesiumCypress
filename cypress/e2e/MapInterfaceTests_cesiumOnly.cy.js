...
describe("tests for", () => {
  beforeEach("Pin by coordinate", () => {
    ...
    // Access Cesium and viewer from the window object
    cy.window().then((win) => {
      Cesium = win.Cesium;
      viewer = win.viewer;
      ...
      // Convert Cartesian3 to screen (window) coordinates
      const screenPosition = Cesium.SceneTransforms.worldToWindowCoordinates(
        scene,
        cartesian
      );

      // Ensure the screen position is valid before clicking
      if (screenPosition) {
        const { x, y } = screenPosition;

        // Perform a click on the calculated screen coordinates
        // cy.getMap().trigger("click", {
        //   clientX: x,
        //   clientY: y,
        // });
  ...
  //   cy.window().then((win) => {
  //     Cesium = win.Cesium; // Access Cesium from the window object
  //     viewer = win.viewer;

  //     const telAviv = Cesium.Cartesian3.fromDegrees(34.7818, 32.0853);
  ...
  //     viewer.camera.flyTo({
  //       destination: Cesium.Cartesian3.fromDegrees(telAviv),
  //       duration: 2, // Fly duration in seconds
  //     });
  ...
  //     const { x, y } = screenPosition; // Extract screen coordinates
