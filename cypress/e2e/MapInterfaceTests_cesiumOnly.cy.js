...
describe("tests for", () => {
  beforeEach("open cesium geo map", () => {
    ...
    cy.wait(5000);
    cy.get("canvas").should("not.be.enabled");

    // it.only("pin on map by geo", () => {
    cy.window().then((win) => {
      ...
      //zoom in
      camera.zoomIn(camera.positionCartographic.height * 0.9);
  ...
  it("checks entity lable", () => {
    cy.log(`entity's lable- ${viewer.entities.values[0].label.text._value}`);
  });

  it("checks entity coordinates", () => {
    const cartographic = Cesium.Cartographic.fromCartesian(
      viewer.entities.values[0].position._value
    );

    const longitude = Cesium.Math.toDegrees(cartographic.longitude);
    const latitude = Cesium.Math.toDegrees(cartographic.latitude);
    const altitude = cartographic.height;
    cy.log(`entity's coordinates- ${longitude}, ${latitude}, ${altitude}`);
  });

  it("checks pinned coordinates", () => {
